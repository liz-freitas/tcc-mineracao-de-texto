...
def gerar_classificador(model, suicidas, nao_suicidas):
...
classificador = gerar_classificador(model, treino_suicida, treino_nao_suicida)
...
# fpr, tpr, thresholds = calc_roc(model, classificador, validacao_suicida, validacao_nao_suicida)
...
while(1):
    ...
    print(vetor)
    resultado = classificador.predict_proba([vetor])
    print('\nResultado: ')
